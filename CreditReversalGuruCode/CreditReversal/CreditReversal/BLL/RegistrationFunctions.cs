    ...
    public class RegistrationFunctions
    {
        ...
        public bool AddAgent(Agent agent)
        {
            ...
            try
            {
                ...
                query += "PrimaryBusinessEmail=@PrimaryBusinessEmail,BillingAdd1=@BillingAdd1,BillingAdd2=@BillingAdd2,BillingCity=@BillingCity,BillingState=@BillingState,BillingZip=@BillingZip,BillingPhone=@BillingPhone,BillingEmail=@BillingEmail,IncChallengeInq=@IncChallengeInq where AgentId =" + id + nl;
                ...
                cmd.Parameters.AddWithValue("@IncChallengeInq", agent.IncChallengeInq == "True" ? "1" : "0");
        ...
        public bool AddBillingInfo(AgentBilling agentBilling, string from = "")
        {
            ...
            try
            {
                ...
                else
                {
                    if (from == "")
                    {
                        string query = "update AgentBilling set BillingType=@BillingType,CardType=@CardType, "
                            + " CardNumber=@CardNumber,ExpiryDate=@ExpiryDate,CVV=@CVV,BillingZipCode=@BillingZipCode,Status=1, "
                            + " CreatedBy=@CreatedBy where AgentId=@AgentId and  isPrimary=1";
                        SqlCommand cmd = new SqlCommand(query);
                        cmd.Parameters.AddWithValue("@AgentId", agentBilling.AgentId);
                        cmd.Parameters.AddWithValue("@BillingType", string.IsNullOrEmpty(agentBilling.BillingType) ? "" : agentBilling.BillingType);
                        cmd.Parameters.AddWithValue("@CardType", string.IsNullOrEmpty(agentBilling.CardType) ? "" : agentBilling.CardType);
                    ...
                    if (from == "Admin")
                    {
                        string query = "update AgentBilling set BillingType=@BillingType,CardType=@CardType,CardNumber=@CardNumber, "
                            + " ExpiryDate=@ExpiryDate,CVV=@CVV,BillingZipCode=@BillingZipCode,Status=1,CreatedBy=@CreatedBy "
                            + " where AgentId='" + agentBilling.AgentId + "'";
                        SqlCommand cmd = new SqlCommand(query);
                        cmd.Parameters.AddWithValue("@AgentId", agentBilling.AgentId);
                        cmd.Parameters.AddWithValue("@BillingType", string.IsNullOrEmpty(agentBilling.BillingType) ? "" : agentBilling.BillingType);
                        cmd.Parameters.AddWithValue("@CardType", string.IsNullOrEmpty(agentBilling.CardType) ? "" : agentBilling.CardType);
        ...
        public int AddAgentNew(Agent agent)
        {
            ...
            try
            {
                using (sqlcon = new SqlConnection(connectionString))
                {
                    ...
                    query = "Insert into Agent (BusinessName,FirstName,LastName,MiddleName,DBA,TypeOfComp,StateOfEncorp,StateOfIncorporationProof,PricingPlan,DateOfEncorp,FedTaxIdentityNo,FedTaxIdentityProof, " + nl;
                    query += "PrimaryBusinessAdd1,PrimaryBusinessAdd2,PrimaryBusinessCity,PrimaryBusinessState,PrimaryBusinessZip,PrimaryBusinessPhone, " + nl;
                    query += "PrimaryBusinessEmail,BillingAdd1,BillingAdd2,BillingCity,BillingState,BillingZip,BillingPhone,BillingEmail,IncChallengeInq,CreatedBy,CreatedDate,DriversLicenseNumber,DriversLicenseState,DriversLicenseCopy) " + nl;
                    query += "values(@BusinessName,@FirstName,@LastName,@MiddleName,@DBA,@TypeOfComp,@StateOfEncorp,@StateOfIncorporationProof,@PricingPlan,@DateOfEncorp,@FedTaxIdentityNo,@FedTaxIdentityProof, " + nl;
                    query += "@PrimaryBusinessAdd1,@PrimaryBusinessAdd2,@PrimaryBusinessCity,@PrimaryBusinessState,@PrimaryBusinessZip,@PrimaryBusinessPhone, " + nl;
                    query += "@PrimaryBusinessEmail,@BillingAdd1,@BillingAdd2,@BillingCity,@BillingState,@BillingZip,@BillingPhone,@BillingEmail,@IncChallengeInq,@CreatedBy,@CreatedDate,@DriversLicenseNumber,@DriversLicenseState,@DriversLicenseCopy) " + nl;
                    query += "SELECT CAST(scope_identity() AS int)" + nl;
