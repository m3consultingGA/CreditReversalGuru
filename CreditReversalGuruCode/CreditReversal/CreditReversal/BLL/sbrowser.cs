    ...
    public class sbrowser
    {
        ...
        public CreditReport pullcredit(string Username, string Password, string SecurityAnswer, string clientid)
        {
            ...
            try
            {
                ...
                List<PulblicRecordPartition>   pulblicRecordPartitions = new List<PulblicRecordPartition>();
                ...
                try
                {
                    TradeLinePartition tradeLinePartition = JsonConvert.DeserializeObject<TradeLinePartition>(rootObj.BundleComponents.BundleComponent[6].TrueLinkCreditReportType.TradeLinePartition.ToString());
                    tdl.Add(tradeLinePartition);
                }
                catch (Exception)
                {
                    List<TradeLinePartition> tradeLinePartitions = JsonConvert.DeserializeObject<List<TradeLinePartition>>(rootObj.BundleComponents.BundleComponent[6].TrueLinkCreditReportType.TradeLinePartition.ToString());
                    tdl.AddRange(tradeLinePartitions);
                }

                try
                {
                    InquiryPartition inquiryPartition = JsonConvert.DeserializeObject<InquiryPartition>(rootObj.BundleComponents.BundleComponent[6].TrueLinkCreditReportType.InquiryPartition.ToString());
                    InquiryPartition.Add(inquiryPartition);
                }
                catch (Exception)
                {
                    List<InquiryPartition> inquiryPartitions = JsonConvert.DeserializeObject<List<InquiryPartition>>(rootObj.BundleComponents.BundleComponent[6].TrueLinkCreditReportType.InquiryPartition.ToString());
                    InquiryPartition.AddRange(inquiryPartitions);
                }
                ...
                try
                {
                    try
                    {
                        PulblicRecordPartition recordPartition = JsonConvert.DeserializeObject<PulblicRecordPartition>(rootObj.BundleComponents.BundleComponent[6].TrueLinkCreditReportType.PulblicRecordPartition.ToString());
                        PublicRecords.AddRange(recordPartition.PublicRecord);
                    }
                    catch (Exception ex)
                    {
                        List<PulblicRecordPartition> recordPartitions = JsonConvert.DeserializeObject<List<PulblicRecordPartition>>(rootObj.BundleComponents.BundleComponent[6].TrueLinkCreditReportType.PulblicRecordPartition.ToString());
                        pulblicRecordPartitions.AddRange(recordPartitions);
                        if (pulblicRecordPartitions.Count > 0)
                        {
                            for (int p = 0; p < pulblicRecordPartitions.Count; p++)
                            {
                                PublicRecords.AddRange(pulblicRecordPartitions[p].PublicRecord);
                            }
                        }
